...
if file is None:
    st.title("Upload Dataset to Start")
    st.stop()

df = pd.read_csv(file)

page = st.sidebar.selectbox("Navigate", [
    "Dashboard", "EDA", "Data Cleaning",
...
# ───────── EDA ─────────
    ...
    fig = px.histogram(num_df, x=col, marginal="box",
                       color_discrete_sequence=["#FF4B4B"])
    st.plotly_chart(fig)

    fig2 = px.imshow(num_df.corr(), text_auto=True, color_continuous_scale="Reds")
...
# ───────── DATA CLEANING ─────────
    ...
    # 🔥 INVALID ZERO UI
    st.markdown("### ⚠️ Handle Invalid Zero Values")
    st.markdown(
        "<span style='color:red'>Select columns where 0 is invalid</span>",
        unsafe_allow_html=True
    )

    invalid_zero_cols = st.multiselect("Columns", num_cols)

    # SHOW ZERO COUNT
    if invalid_zero_cols:
        zero_counts = {
            col: int((df[col] == 0).sum()) for col in invalid_zero_cols
        }
        st.write("Detected Zero Values")
        st.dataframe(pd.Series(zero_counts).reset_index().rename(
            columns={"index":"Column",0:"Zero Count"}))

    st.markdown("---")

    # OPTIONS
    missing_option = st.selectbox("Missing", ["None","Mean","Median","Drop"])
    outlier_option = st.selectbox("Outlier", ["None","IQR"])
    ...
    if st.button("Apply Cleaning"):
        ...
        if outlier_option=="IQR":
            ...
            IQR=Q3-Q1
            df_clean=df_clean[~((df_clean[num_cols]<(Q1-1.5*IQR))|
...
# ───────── FEATURE SELECTION ─────────
    ...
    target = st.selectbox("Target", df_used.columns)
    ...
    if method=="Random Forest":
        ...
        top=st.slider("Top N",1,len(X.columns),5)
        selected=imp.sort_values(ascending=False).head(top).index.tolist()
        ...
        top=st.slider("Top N",1,len(X.columns),5)
        selected=corr.sort_values(ascending=False).head(top).index.tolist()
...
# ───────── MODEL TRAINING ─────────
    ...
    df_used = st.session_state.get("df_clean", df)
    X = df_used[st.session_state["features"]]
    y = df_used[st.session_state["target"]]

    task = detect_task_type(y)

    if task=="classification":
        y,_=encode_target(y)
        model_type=st.selectbox("Model",["Logistic","RF"])
    else:
        model_type=st.selectbox("Model",["Linear","RF"])

    test_size=st.slider("Test Size",0.1,0.4,0.2)
    k=st.slider("KFold",2,10,5)
    ...
    if st.button("Train"):
        ...
        if task=="classification":
            model=LogisticRegression() if model_type=="Logistic" else RandomForestClassifier()
        else:
        ...
        if task=="classification":
            ...
            st.metric("F1",f1_score(y_test,pred,average="weighted"))

            cv=StratifiedKFold(n_splits=k)
            scores=cross_validate(model,scaler.fit_transform(X),y,cv=cv,scoring="accuracy")
            st.metric("CV Avg",scores["test_score"].mean())

            cm=confusion_matrix(y_test,pred)
            st.plotly_chart(px.imshow(cm,text_auto=True))

        else:
            st.metric("R2",r2_score(y_test,pred))
            st.metric("RMSE",np.sqrt(mean_squared_error(y_test,pred)))
            st.metric("MAE",mean_absolute_error(y_test,pred))

            cv=KFold(n_splits=k)
            scores=cross_validate(model,scaler.fit_transform(X),y,cv=cv,scoring="r2")
            st.metric("CV Avg R2",scores["test_score"].mean())
